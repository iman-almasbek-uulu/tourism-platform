...
const Header = () => {
    ...
    const culture = [
        { name: t("Национальные инструменты", "الآلات الوطنية", "National Instruments"), path: "/culture/instruments" },
        { name: t("Национальная одежда", "الملابس الوطنية", "National Clothes"), path: "/culture/clothes" },
        { name: t("Ручная работа", "الحرف اليدوية", "Hand Crafts"), path: "/culture/crafts" },
    ...
    const navItems = [
        ...
        { name: { ru: "Регионы", ar: "المناطق", en: "Regions" }, path: `/${isRegionName}` },
                        ...
                        <nav>
                            {navItems.map((item,i) => {
                                if (item.name.ru === "Регионы") {
                                    return <div
                                        key={i}
                                        className={`${isActive(item.path) ? scss.active : ""} ${scss.link}`}
                                        onClick={() => {
